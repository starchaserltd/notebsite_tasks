...
if($result&&mysqli_num_rows($result)>0)
{
	...
	{			
		$sql="SELECT max(`price`) as `max_price`,min(`price`) as `min_price`,max(`batlife`) as `max_batlife`,min(`batlife`) as `min_batlife`,max(`capacity`) as `max_cap`,min(`capacity`) as `min_cap` FROM `all_conf_".$model['model_id']."` WHERE `price`!=0 LIMIT 1";
