...
function calculate_conf_data($conf,$comp_list,$new_price_calc=NULL)
{
	...
	if($to_return!==NULL)
	{
		...
		$cpu_float_tdp=floatval($comp_data["cpu"]["tdp"]);
		if(stripos($comp_data["cpu"]["prod"],"ARM")!==FALSE){ $cpu_float_tdp=$cpu_float_tdp/3; }
		$bat_com=$bat_com+round((0.5+($cpu_float_tdp/7)),5);
