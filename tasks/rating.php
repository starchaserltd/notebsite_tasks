...
/* CPU RATING */
$abort=True;
$SQL_COLUMNS="SELECT * FROM `notebro_rate`.`CONFIG` WHERE `type`='cpu_columns' LIMIT 1";
...
if(have_results($bench_name_result))
{
	...
	$SQL_MAX_BENCH_DATA="SELECT ".implode(",",$SQL_PARTS)." FROM `notebro_rate`.`CPU` WHERE 1=1 LIMIT 1";
	unset($SQL_PARTS);
	...
	if(!$abort)
	{
		echo "Updating CPU ratings.<br>";
		$SQL_BENCH_DATA="SELECT `id`,`".implode("`,`",$bench_names)."` FROM `notebro_rate`.`CPU`";
		...
		if(have_results($result))
		{
			while($row=mysqli_fetch_assoc($result))
			{
				...
				$SQL_UPDATE="UPDATE `notebro_rate`.`CPU` SET `ratingnew`=".$value." WHERE id='".$row["id"]."'";
				#var_dump($SQL_UPDATE); echo "<br>";
				mysqli_query($con,$SQL_UPDATE);
				mysqli_select_db($con,"notebro_db");
				$SQL_UPDATE="UPDATE `notebro_db`.`CPU` SET `rating`=".$value." WHERE id='".$row["id"]."'";
				mysqli_query($con,$SQL_UPDATE);
				mysqli_select_db($con,"notebro_rate");
			}
			mysqli_free_result($result);
